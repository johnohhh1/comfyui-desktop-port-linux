...
describe('PathHandlers', () => {
  ...
  describe('get-system-paths', () => {
    ...
    it('prefers an existing ComfyUI install in the user home directory', async () => {
      const existingInstallPath = path.join('/mock/home', 'ComfyUI');
      vi.mocked(ComfyConfigManager.isComfyUIDirectory).mockImplementation((candidatePath) => {
        return candidatePath === existingInstallPath;
      });

      const result = await getSystemPathsHandler({});
      expect(result.defaultInstallPath).toBe(existingInstallPath);
    });
