...
export class ComfyDesktopApp implements HasTelemetry {
  ...
  getExternalServerSettings(overrides: { useExternalServer: boolean; COMFY_HOST?: string; COMFY_PORT?: string }) {
    const externalServerHost = useDesktopConfig().get('externalServerHost');
    const externalServerPort = useDesktopConfig().get('externalServerPort');

    return {
      useExternalServer: overrides.useExternalServer || useDesktopConfig().get('useExternalServer', false),
      host: overrides.COMFY_HOST ?? externalServerHost,
      port: overrides.COMFY_PORT ?? externalServerPort,
    };
  }

  shouldUseExternalServer(overrides: DevOverrides) {
    return this.getExternalServerSettings(overrides).useExternalServer;
  }
  ...
  async buildServerArgs({ useExternalServer, COMFY_HOST, COMFY_PORT }: DevOverrides): Promise<ServerArgs> {
    const externalServerSettings = this.getExternalServerSettings({ useExternalServer, COMFY_HOST, COMFY_PORT });
    ...
    if (externalServerSettings.host) serverArgs.listen = externalServerSettings.host;
    if (externalServerSettings.port) serverArgs.port = externalServerSettings.port;

    // Find first available port (unless using external server).
    if (!externalServerSettings.useExternalServer) {
