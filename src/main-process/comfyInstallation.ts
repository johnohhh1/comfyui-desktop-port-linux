...
export class ComfyInstallation {
  ...
  get usesExternalServer() {
    return useDesktopConfig().get('useExternalServer', false);
  }
  ...
  async validate(): Promise<DesktopInstallState> {
    ...
    // External-server mode reuses an existing ComfyUI installation and does not
    // depend on the desktop-managed Python environment.
    if (validation.basePath !== 'error' && this.usesExternalServer) {
      validation.venvDirectory = 'skipped';
      validation.pythonInterpreter = 'skipped';
      validation.uv = 'skipped';
      validation.pythonPackages = 'skipped';
      validation.git = 'skipped';
      validation.vcRedist = 'skipped';
      this.onUpdate?.(validation);
    } else if (validation.basePath !== 'error') {
