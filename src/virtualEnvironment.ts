...
export class VirtualEnvironment implements HasTelemetry, PythonExecutor {
  ...
  public async installRequirements(callbacks?: ProcessCallbacks): Promise<void> {
    ...
    if (!(await pathAccessible(this.requirementsCompiledPath))) {
      log.warn(`Compiled requirements not found at ${this.requirementsCompiledPath}. Falling back to manual install.`);
