...
type DesktopGpuType = Extract<GpuType, 'nvidia' | 'amd' | 'mps'>;
...
function getDesktopGpuFromController(controller: Systeminformation.GraphicsControllerData): DesktopGpuType | undefined {
  const vendorId = normalizeVendorId(controller.vendorId);
  if (vendorId === NVIDIA_VENDOR_ID) return 'nvidia';
  if (vendorId === AMD_VENDOR_ID) return 'amd';
  ...
  if (details.includes('nvidia')) return 'nvidia';
  if (details.includes('amd') || details.includes('radeon') || details.includes('advanced micro devices')) return 'amd';
  if (details.includes('apple')) return 'mps';
  return undefined;
}

function getDesktopGpuFromGraphics(graphics: Systeminformation.GraphicsData): DesktopGpuType | undefined {
  for (const controller of graphics.controllers) {
    const detected = getDesktopGpuFromController(controller);
    if (detected) return detected;
  }
  return undefined;
}
...
export async function validateHardware(): Promise<HardwareValidation> {
  ...
  try {
    ...
    if (process.platform === 'linux') {
      const graphics = await si.graphics();
      const detectedGpu = getDesktopGpuFromGraphics(graphics);

      if (process.env.SKIP_HARDWARE_VALIDATION) {
        console.log('Skipping hardware validation');
        return detectedGpu ? { isValid: true, gpu: detectedGpu } : { isValid: true };
      }

      if (detectedGpu) return { isValid: true, gpu: detectedGpu };
      if (await hasNvidiaGpuViaSmi()) return { isValid: true, gpu: 'nvidia' };

      // Linux support defaults to CPU mode when no supported GPU is detected.
      return { isValid: true };
    }
