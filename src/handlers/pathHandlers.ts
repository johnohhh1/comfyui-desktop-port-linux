...
const getPreferredDefaultInstallPath = (documentsPath: string): string => {
  const fallbackPath = path.join(documentsPath, 'ComfyUI');
  const candidatePaths = [path.join(app.getPath('home'), 'ComfyUI'), fallbackPath];

  for (const candidatePath of candidatePaths) {
    if (ComfyConfigManager.isComfyUIDirectory(candidatePath)) {
  ...
  ipcMain.handle(IPC_CHANNELS.GET_SYSTEM_PATHS, (): SystemPaths => {
    ...
    if (process.platform === 'win32') {
      ...
      return {
        ...
        defaultInstallPath: getPreferredDefaultInstallPath(documentsPath),
      };
    }
