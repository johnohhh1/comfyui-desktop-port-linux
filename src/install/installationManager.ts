...
const EXTERNAL_SERVER_HOST = '127.0.0.1';
const EXTERNAL_SERVER_PORT = '8188';
...
export class InstallationManager implements HasTelemetry {
  ...
  async ensureInstalled(): Promise<ComfyInstallation> {
    ...
    // Fresh install
    if (!installation) {
      const externalInstallation = await this.bootstrapExistingExternalInstallation();
      if (externalInstallation) return externalInstallation;
  ...
  private async bootstrapExistingExternalInstallation(): Promise<ComfyInstallation | undefined> {
    const basePath = this.findExistingComfyInstallationPath();
    if (!basePath) return undefined;

    const externalServerReady = await this.canAccessExternalServer();
    if (!externalServerReady) return undefined;

    log.info(
      `Detected existing ComfyUI install at ${basePath} with running external server on ${EXTERNAL_SERVER_PORT}`
    );

    const config = useDesktopConfig();
    config.set('basePath', basePath);
    config.set('installState', 'installed');
    config.set('useExternalServer', true);
    config.set('externalServerHost', EXTERNAL_SERVER_HOST);
    config.set('externalServerPort', EXTERNAL_SERVER_PORT);

    await ComfySettings.load(basePath);
    return new ComfyInstallation('installed', basePath, this.telemetry);
  }

  private findExistingComfyInstallationPath(): string | undefined {
    const candidatePaths = [app.getPath('home'), app.getPath('documents')].map((basePath) =>
      path.join(basePath, 'ComfyUI')
    );
    return candidatePaths.find((candidatePath) => ComfyConfigManager.isComfyUIDirectory(candidatePath));
  }

  private async canAccessExternalServer(): Promise<boolean> {
    try {
      const response = await fetch(`http://${EXTERNAL_SERVER_HOST}:${EXTERNAL_SERVER_PORT}/queue`, {
        signal: AbortSignal.timeout(1500),
      });
      return response.ok;
    } catch (error) {
      log.info('No compatible external ComfyUI server detected.', error);
      return false;
    }
  }
  ...
  async freshInstall(): Promise<ComfyInstallation> {
    ...
    useDesktopConfig().set('useExternalServer', false);
    useDesktopConfig().delete('externalServerHost');
    useDesktopConfig().delete('externalServerPort');
